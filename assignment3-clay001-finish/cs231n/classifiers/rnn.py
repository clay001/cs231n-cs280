...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        now_h, _ = affine_forward(features, W_proj, b_proj) 
        if self.cell_type == 'lstm':
            prev_c = np.zeros_like(now_h)
 
        word_embed , _ = word_embedding_forward(self._start,W_embed)
        
        for i in range(max_length):
            if self.cell_type == 'rnn':
                now_h, cache_h = rnn_step_forward(word_embed,now_h,Wx,Wh,b)
            else: 
                now_h,prev_c, _  = lstm_step_forward(word_embed, now_h,prev_c, Wx, Wh, b) 
                
            out, _ = affine_forward(now_h, W_vocab, b_vocab)
